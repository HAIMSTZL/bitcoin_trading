...
它只会写入独立的 `trading/data/trading_predictive.db`，并且代码级禁止在 `live` 模式运行。预测
...
特征，以过去 120 天、标签已经兑现的数据滚动训练正则化回归模型，预测未来收益。
信号只在**下一根 K 线开盘**成交，初始状态完全是 USDT；预测没有覆盖成本就保持空仓。每侧
默认还扣除 0.1% 手续费和 3 bps 滑点。

历史回测中的“下一根开盘”是可复现的 K 线代理价格；预测模拟盘则在发现新的已收盘 K 线后，使用
实际读取的当前 ticker 加保守滑点成交。每次预测决策、ticker 读取时间和虚拟成交都会落库，不能把
两种口径视为完全相同，前向模拟应据此测量实际延迟与价格偏离。
